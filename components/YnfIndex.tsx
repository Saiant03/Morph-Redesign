...
import { type Offer, lei, firstSentences, offerItem } from '@/lib/catalog';
  ...
  return (
    <div className={s.split}>
      <ol className={s.list} aria-label="Seturile Your Next Form">
        {sets.map(o => (
          <li key={o.slug} className={s.row} data-active={o.slug === active} onMouseEnter={() => setActive(o.slug)} onFocus={() => setActive(o.slug)}>
            <Niche src={o.image} alt="" sizes="72px" className={s.thumb} />
            ...
            <div className={s.act}>
              {o.inStock
                ? <AddToCart className="btn btn-sm btn-secondary" items={[offerItem(o, `Your Next Form ${o.state}`, 'Set blind 2×8 ml')]} aria-label={`Adaugă setul ${o.state} în coș, ${lei(o.price)}`}>Adaugă</AddToCart>
      ...
      <aside className={s.preview} aria-label={`Previzualizare ${a.state}`}>
        <Niche src={a.image} alt={`Cutia setului ${a.state}`} sizes="34vw" className={s.previewVisual} />
        <h2 className={s.previewName}>{a.state}</h2>
        <p>{accord(a)}</p>
        <p className="t-small muted">Set blind, 2×8 ml. Parfumurile se dezvăluie la deschiderea cutiei.</p>
        <div className={s.previewActions}>
          {a.inStock && <AddToCart items={[offerItem(a, `Your Next Form ${a.state}`, 'Set blind 2×8 ml')]}>Adaugă în coș <span className="num">{lei(a.price)}</span></AddToCart>}
