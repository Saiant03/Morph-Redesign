...
export function ProductCard({ p, sizes = '(max-width: 899px) 72vw, 25vw', vt, onPick }: { p: Perfume; sizes?: string; vt?: boolean | 'idle'; onPick?: () => void }) {
  return (
    <article className={`${s.card} ${p.inStock ? '' : s.soldout}`}>
      <Link href={productHref(p)} className={s.cardLink} onClick={onPick}>
