...
// Ice is a crop of the Morph image associated with Primitivo (data/assets.json). One alt serves both crops, so it
// names only what both show: the phone crop has no sand.
export const CAMPAIGN: Record<CollectionId, { src: string; mobile: string; alt: string; pos: string; posM: string; side: 'left' | 'right'; align?: 'end' }> = {
  ...
  ice: { src: '/morph/campaign/ice-campaign.avif', mobile: '/morph/campaign/ice-campaign-m.avif', alt: 'Imaginea Morph a colecției Ice: mâini în mănuși de laborator țin o fâșie de testare și sticla Primitivo', pos: '70% 6%', posM: '50% 22%', side: 'right', align: 'end' },
};
