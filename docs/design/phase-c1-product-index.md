...
2. **Hard white line at the lower left of the bottle.** First fix: the streak faded out between 55% and 78% of the packshot's height. The line was still visible on the home hero (Zeta), so the cause was fixed at the mask. The streak also crossed the clear glass shell around the juice. That shell is near-white in the packshot, reads as empty on a dark stage, and turned the streak into a floating white line. Two changes, which apply everywhere an object is lit (stage, vitrine, niches):
   - `scripts/lightmask.mjs` now leaves out near-white pixels inside the outline, so the light falls only on juice, cap and label. All 26 masks were regenerated, with a 2 px feather.
   - `ObjectLight` intersects the silhouette with a vertical fade from 38% to 66% of the packshot's height (`mask-composite: intersect`). The key light comes from behind and above, so the streak sits on the shoulders and is gone before the base.
   - Cost: on the pale Ice bottles, whose upper juice is almost white, the streak is fainter.
